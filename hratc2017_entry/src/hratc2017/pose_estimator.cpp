...
PoseEstimator::PoseEstimator(ros::NodeHandle* nh)
  : ROSNode(nh, 30), initial_pose_sent_(false), current_state_(states::S1_READING1),
    has_utm_reading1_(false), has_utm_reading2_(false), isMoving_(false)
{
  ...
  initial_orientation_pub_ = nh->advertise<geometry_msgs::PoseWithCovarianceStamped>("/initialOrietation", 100, true);
...
PoseEstimator::~PoseEstimator()
{
  ...
  initial_orientation_pub_.shutdown();
...
{ 
  if (!initial_pose_sent_ && current_state_!=states::S2_MOVING)
  {
    ROS_INFO("READINGS...");
    if(has_utm_reading1_ && !has_utm_reading2_){
      ROS_INFO("READING 1");
      p1_.pose = utm_reading1_.pose;
    ...
    else if(has_utm_reading2_){
      ROS_INFO("READING 2");
      p2_.pose = utm_reading2_.pose;
      p2_.pose.pose.position.x = utm_reading1_.pose.pose.position.x - centerX_;
      p2_.pose.pose.position.y = utm_reading1_.pose.pose.position.y - centerY_;
      ...
      initial_orientation_pub_.publish(p2_);
      initial_pose_sent_ = true;
      ROS_INFO("Published initial pose (x=%f, y=%f, z=%f)", p2_.pose.pose.position.x, p2_.pose.pose.position.y, p2_.pose.pose.position.z);
    }
  }
}
